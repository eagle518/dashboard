...
/**
 * 角色管理的服务类,由于采用了EXT0配置框架，方法执行完后会默认返回到以类名-方法名.xml命名的xml配置页面
 * @author 毛子源
 *
 */
public class RoleService extends ServiceSupport
{
	 ...
	 * @param option
	 */
	public void roleReportList(InputOption option)
	{
		Map param = new HashMap();
		String roleId = option.getParamValue("role_id");
		param.put("roleId", roleId);
		param.put("dbName", VDOPUtils.getConstant(ExtConstants.dbName));
		List ls = daoHelper.getSqlMapClientTemplate().queryForList("vdop.role.rolereport", param);
		TreeService tree = new TreeService();
		List ret = tree.createTreeData(ls, new TreeInterface(){
			@Override
	 ...
	 * @param option
	 */
	public void roleDataList(InputOption option){
		Map param = new HashMap();
		String roleId = option.getParamValue("role_id");
		param.put("dbName", VDOPUtils.getConstant(ExtConstants.dbName));
		param.put("roleId", roleId);
		List ls = daoHelper.getSqlMapClientTemplate().queryForList("vdop.role.roledata", param);
		TreeService tree = new TreeService();
		List ret = tree.createTreeData(ls, new TreeInterface(){
			@Override
			public void processMap(Map m) {
	...
	{
		String userid = VDOPUtils.getLoginedUser(option.getRequest()).getUserId();//获取用户ID
		String role_id = option.getRequest().getParameter("role_id");//获取角色ID
		RoleMenuParm rmp = new RoleMenuParm(userid,null,role_id);//封装参数信息
		List ls = this.daoHelper.getSqlMapClientTemplate().queryForList("vdop.role.rolemenu",rmp);
		TreeService tree = new TreeService();
		List ret = tree.createTreeData(ls, new TreeInterface(){
			@Override
			public void processMap(Map m) {
