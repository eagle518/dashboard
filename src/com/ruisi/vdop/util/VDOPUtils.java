...
public final class VDOPUtils {
	...
	public static User getLoginedUser(){
		...
		User u=null;
		if(session!=null){
			u=(User)session.getAttribute(VdopConstant.USER_KEY_IN_SESSION);
		}
		return u;
	}
	 ...
	 * @return
	 */
	public static void saveLoginedUser(User user, HttpServletRequest req, ServletContext ctx){

		HttpSession session=getSession();
		if(session==null)session=getSession(true);
		session.setAttribute(VdopConstant.USER_KEY_IN_SESSION,user);
		//添加登录用户数
	...
	public static void removeLoginUser(ServletContext ctx, HttpSession s, boolean is3G){
		...
		User u=(User)s.getAttribute(VdopConstant.USER_KEY_IN_SESSION);
	...
	public static String getAppUserId(){
		String uid = (String)VDOPUtils.getRequest().getAttribute("app.userid");
		return uid;
	}
}
