			...
			if(isfirst == false){
				if(data.length > 0){
					...
					for(i=0; i<data.length; i++){
						if(data[i].id == submenuId){
							var u = data[i].attributes.url;
					...
					if(!exist){
						var u;
						var root = jQuery("#sysmenu").tree("getRoots")[0];
						var ls = jQuery("#sysmenu").tree("getChildren", root.target);
						if(ls.length == 0){
							$("#sysmenu").tree("select", root.target);
							u = root.attributes.url;
						}else{
							$("#sysmenu").tree("expand", root.target);
							var cld = $("#sysmenu").tree("getChildren", root.target)[0];
							$("#sysmenu").tree("select", cld.target);
							u = cld.attributes.url;
						}
						document.getElementById("syspageinfo").src = u;
					}
				}
